...
class NoteServiceTest {
    ...
    void findByUser_유저가_게시물조회() {
        // given
        User user = userRepository.save(new User("username", "user",GenderType.MALE,22, "ROLE_USER"));
        noteRepository.save(new Note("title1", "content1", user));
        noteRepository.save(new Note("title2", "content2", user));
        // when
        List<Note> notes = noteService.findByUserNote(user);
        // then
    ...
    void findByUser_어드민이_조회() {
        // given
        User admin = userRepository.save(new User("admin123", "admin",  GenderType.MALE,22, "ROLE_ADMIN"));
        User user1 = userRepository.save(new User("username", "user",GenderType.MALE,22, "ROLE_USER"));
        User user2 = userRepository.save(new User("username2", "user",GenderType.MALE,22, "ROLE_USER"));
        ...
        // when
        List<Note> notes = noteService.findByUserNote(admin);
        // then
    ...
    void saveNote() {
        // given
        User user = userRepository.save(new User("user123", "user",GenderType.MALE,22, "ROLE_USER"));
        // when
